...
export default {
  ...
  methods: {
    onElementClick (event) {
      ...
      let target = Tools.searchIdTarget(event.target)
      console.log(target.id)
      this.$emit('onElementClick', target.id)
      // return false
    },
    onMouseOver (event) {
      event.preventDefault()
      event.stopPropagation()
      Tools.searchIdTarget(event.target).style.border = 'solid 3px #ff9000'
    },
    onMouseOut (event) {
      event.preventDefault()
      event.stopPropagation()
      Tools.searchIdTarget(event.target).style.border = ''
    },
    mockTransLang (target) {
      if (target instanceof Object) {
        return target['zh-cn'] || target['zh-cn'] || ''
      } else {
        return target || ''
      }
    },
    mockDataSource () {
      ...
      for (let item of DataSources) {
        dataSource[item.id] = item.exampleData
      }
      return dataSource
    },
    // 标签渲染选择器
    confRender (h, conf, ajaxData) {
      ...
      if (conf.dataSource) {
        if (conf.singleData) {
          ajaxData = this.mockDataArray[conf.dataSource][0]
        } else {
          ajaxData = this.mockDataArray[conf.dataSource]
        }
        console.log(conf.type, ajaxData)
      }
      let rs = {}
      switch (conf.type) {
        case 'Layout':
    ...
    gridColumnRender (h, conf, ajaxData, defaultStyle) {
      ...
      return h(
        'grid-column',
        {
          props: {
            prop: conf.name,
            label: typeof conf.label === 'number' ? String(conf.label) : (this.mockTransLang(conf.label) || ''),
            width: style.width,
  ...
  created () {
    this.mockDataArray = this.mockDataSource()
  },
  mounted () {
    console.log('mounted')
  },
  render (h) {
    console.log(Tools.deepCopy(this.templateConf))
