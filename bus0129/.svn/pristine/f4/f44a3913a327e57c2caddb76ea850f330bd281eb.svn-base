        ...
        <div>
          ...
          <el-form-item>
            <span class="editor-label">
              ...
              <el-switch v-model="switchOptions.backgroundColor.switch" active-color="#13ce66" inactive-color="#ff4949" ></el-switch>
            </span>
            <div v-show="switchOptions.backgroundColor.switch" class="editor-inner-pannel editor-inner-pannel-left">
              <color-picker v-model="switchOptions.backgroundColor.value" :showLabel="false" placeholder="选择颜色" :inputScale="20"></color-picker>
            </div>
          </el-form-item>
           <el-form-item>
            <span class="editor-label">
              对齐方式(textAlign)
              <el-switch v-model="switchOptions.textAlign.switch" active-color="#13ce66" inactive-color="#ff4949" ></el-switch>
            </span>
            <div v-show="switchOptions.textAlign.switch" class="editor-inner-pannel editor-inner-pannel-left">
              <selector v-model="switchOptions.textAlign.value" :options="Tools.alignOptions()" filterable :showLabel="false" placeholder="选择对齐方向"></selector>
            </div>
          </el-form-item>
          <el-form-item>
            <span class="editor-label">
            ...
            <div v-show="switchOptions.borderRadius.switch" class="editor-inner-pannel editor-inner-pannel-left">
              <el-input type="number" :min="0" :value="switchOptions.borderRadius.value" @input="val => Tools.inputStr2Number(switchOptions.borderRadius, 'value', val)"></el-input>
            </div>
          </el-form-item>
        </div>
      </fieldset>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      'switchOptions.backgroundColor': {
        handler (val) {
          Tools.operateSwitch(val.switch, this.conf.style, 'backgroundColor', val.value)
        },
        deep: true
      },
      ...
      'switchOptions.textAlign': {
        handler (val) {
          Tools.operateSwitch(val.switch, this.conf.style, 'textAlign', val.value)
        },
        deep: true
      }
    },
    ...
    methods: {
      initSwitch () {
        ...
        // backgroundColor
        if (this.conf.style && this.conf.style.backgroundColor) {
          this.setSwitchElements(this.switchOptions.backgroundColor, this.conf.style.backgroundColor)
        }
        // borderRadius
        if (this.conf.style && this.conf.style.borderRadius > -1) {
          this.setSwitchElements(this.switchOptions.borderRadius, this.conf.style.borderRadius)
        }
        // textAlign
        if (this.conf.style && this.conf.style.textAlign) {
          this.setSwitchElements(this.switchOptions.textAlign, this.conf.style.textAlign)
        }
      },
