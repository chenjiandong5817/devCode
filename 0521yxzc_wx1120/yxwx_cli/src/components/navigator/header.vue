<template>
  <header>
    ...
    <div class="citySelectMask" v-show="show">
      <ul>
        <li v-for="cityItem in openedCities" :key="cityItem.uuid" @click="set_currentCity({city: cityItem.cityNames,areaUuid: cityItem.sysAreaConfigDto.areaUuid})">{{ cityItem.cityNames }}</li>
      </ul>
      <!-- <ul>
...
export default{
  ...
  methods: {
    ...
    getOpenedCities () {
      this.openedCities = api.getLocalStorage('OPENED_ARPORT', 'object')
      for (var c = 0; c < this.openedCities.length; c++) {
        this.openedCities[c].cityNames = this.openedCities[c].cityNames.substring(0, 2) + '市'
        if (this.openedCities[c].cityNames === '泉州市') {
          this.openedCities[c].cityNames = '厦门市'
        }
      }
    }
  },
