  ...
  <el-dialog :title="title" :visible.sync="visible" :close-on-click-modal="false" :before-close="handleBeforeClose" @close="handleClose" :size="size" >
    <el-form :model.sync="target" :label-width="labelWidth + 'px'" :rules="rules" ref="commonForm">
      <el-form-item v-for="item in fields" :label="item.label" :prop="item.name" :key="item.id" v-if="!item.hidden">
        ...
        <el-button type="primary" v-on:click="getClickEvent" v-else-if="item.type === 'button'">经纬度定位</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    getClickEvent () {
      this.$emit('getClickEvent', this.target)
    },
    ...
    // 点击确定之后的回调
    submitEmit () {
      this.$emit('submitEmit', this.targetCache)
    },
    // 回调函数的时候发送信息
    remoteEditCallBack () {
      this.$emit('remoteEditCallBack', this.targetCache)
    },
    ...
    handleSubmit: function () {
      ...
      if (this.target.password) {
        this.target['errortimes'] = 0
        this.target['lasterrortime'] = ''
      }
      // 用于远程控制的
      if (this.target.deviceid) {
        if (this.target.lockAddress === '') {
          this.target['islock'] = '0'
        }
        if (this.target.lockAddress !== '') {
          this.target['islock'] = '1'
        }
      }
      this.targetCache = util.deepCopy(this.target)
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            this.to(para).then((res) => {
              ...
              this.$notify(util.notifyBody(res.status, res.message))
              this.$refs['commonForm'].resetFields()
              this.visible = false
              this.fields = []
              // 如果是远程控制的页面就执行相关函数
              if (this.targetCache.deviceid) {
                this.remoteEditCallBack()
                // return
              } else {
                this.callback()
                this.callbackother()
