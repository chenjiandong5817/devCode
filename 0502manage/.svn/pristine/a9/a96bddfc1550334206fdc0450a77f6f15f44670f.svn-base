	          ...
	          <el-row>
	             <el-col>
                    ...
                    <el-upload
                      ...
                      <el-dialog :visible.sync="dialogVisible" :before-close="handleImgClose">
        ...
        <el-tab-pane label="第三广告位" name="thirdAdd">
          <div class="third_content" v-if="thirdAdd">
            ...
            <el-row class="pay_class_third">
              ...
              <el-col :span="5">
                  <el-input v-model="aliPayParam" placeholder="请输入变更的广告语" @change="inputChange" size="large"></el-input>
              </el-col>
              <el-col :span="2" style="margin-left: 15px">
                  <el-button size="large" type="primary" @click="editAdContent">提交更改</el-button>
              </el-col>
            </el-row>
            <!-- 第四行微信 -->
            <el-row class="pay_class_third">
              ...
              <el-col :span="5">
                  <el-input v-model="weChatParam" placeholder="请输入变更的广告语" @change="inputChange" size="large"></el-input>
              </el-col>
              <el-col :span="2" style="margin-left: 15px">
                  <el-button size="large" type="primary" @click="editAdContent">提交更改</el-button>
              </el-col>
            ...
            <el-row class="pay_class_third">
              ...
              <el-col :span="5">
                 <el-input v-model="unionPayParam" placeholder="请输入变更的广告语" @change="inputChange" size="large"></el-input>
              </el-col>
              <el-col :span="2" style="margin-left: 15px">
                 <el-button size="large" type="primary" @click="editAdContent">提交更改</el-button>
              </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      clickTag (value) {
        ...
        // 只有第三组能够获得广告位
        if (this.operateAdd === 'thirdAdd') {
          // 清空内容
          this.aliPayParam = ''
          this.weChatParam = ''
          this.unionPayParam = ''
          this.getAllAdContent()
        }
      },
      // 给颜色
      getColor () {
              ...
              if (data.status === 1) {
                for (var i = 0; i < data.data.length - 1; i++) {
                  for (var j = 0; j < data.data.length - i - 1; j++) {
                    if (parseInt(data.data[j].num) > parseInt(data.data[j + 1].num)) {
                      var swap = data.data[j]
      ...
      addGroup () {
        ...
        if (this.webTags.length === 1) {
          max = (parseInt(this.webTags[0].num) + 1).toString()
        }
        if (this.webTags.length > 1) {
          max = parseInt(this.webTags[0].num)
          for (var c = 0; c < this.webTags.length; c++) {
            if (parseInt(this.webTags[c].num) > max) {
              max = parseInt(this.webTags[c].num) + 1
            }
          }
          max = (max + 1).toString()
        }
        // 上传的参数
        var param = { id: '', num: max, type: this.groupType }
        // 调用接口
            ...
            API.getAdGroup().go(this.groupType).then((data) => {
              if (data.status === 1) {
                ...
                if (this.groupAllData) {
                  this.clickTag(this.groupAllData)
                  // 获取到新加的组的id
                  for (var a = 0; a < this.webTags.length; a++) {
                    if (this.webTags[a].num === param.num) {
                      if (this.operateAdd === 'thirdAdd') {
                        this.addAdContent(this.webTags[a].id)
                      }
                    }
                  }
                } else {
                  this.clickTag(this.webTags[0])
                  if (this.operateAdd === 'thirdAdd') {
                    this.addAdContent(this.webTags[0].id)
                  }
                }
              } else {
                this.$notify(Util.notifyBody(false, data.message))
      ...
      beforeUpload (file) {
        ...
        // 获得比最大的picNUM大一
        for (var i = 0; i < this.fileListGetted.length - 1; i++) {
          for (var j = 0; j < this.fileListGetted.length - i - 1; j++) {
            if (parseInt(this.fileListGetted[j].pictureNum) > parseInt(this.fileListGetted[j + 1].pictureNum)) {
              var swap = this.fileListGetted[j]
              this.fileListGetted[j] = this.fileListGetted[j + 1]
              this.fileListGetted[j + 1] = swap
            }
          }
        }
      ...
      forbiddenCheckbox (sels) {
        ...
        this.$nextTick(() => {
          ...
          var labelCheckBoxs = document.getElementsByClassName('el-checkbox')
          ...
          labelCheckBoxs[0].style.pointerEvents = 'auto'
          allCells[0].style.cursor = 'default'
          // 筛选判断
          if (forbiddenIndex.length !== 0) {
            // allSpans[0].style.cursor = 'not-allowed'
            // allCheckBoxs[0].setAttribute('disabled', true)
            // 让其失去点击事件
            labelCheckBoxs[0].style.pointerEvents = 'none'
            // 体现禁用鼠标手
            allCells[0].style.cursor = 'not-allowed'
          }
      ...
      // 获取所有广告标语
      getAllAdContent () {
        var groupid = this.groupAllData.id
        API.getAllAdContent().go(groupid).then((data) => {
          if (data.status === 1) {
            if (data.data) {
              this.allAdContent = data.data
              this.aliPayParam = data.data.alipayword
              this.weChatParam = data.data.wechatpayword
              this.unionPayParam = data.data.unionpayword
            }
            this.getColor()
          } else {
            this.$notify(Util.notifyBody(false, data.message))
      ...
      // 删除该组广告标语
      delAdContent () {
        var groupid = this.delData.id
        API.removeAllAdContent().go(groupid).then((data) => {
          if (data.status === 1) {
            // this.$notify(Util.notifyBody(true, data.message))
            this.getColor()
          } else {
      ...
      // 修改该组广告标语
      editAdContent () {
        var params = { id: this.allAdContent.id, groupid: this.groupAllData.id, alipayword: this.aliPayParam, wechatpayword: this.weChatParam, unionpayword: this.unionPayParam }
        API.editAllAdContent().go(params).then((data) => {
          if (data.status === 1) {
            this.getAllAdContent()
            this.$notify(Util.notifyBody(true, data.message))
            this.getColor()
      ...
      // 添加广告标语
      addAdContent (groupid) {
        var params = { groupid: groupid, alipayword: '', wechatpayword: '', unionpayword: '' }
        API.addAllAdContent().go(params).then((data) => {
          if (data.status === 1) {
            // this.$notify(Util.notifyBody(true, data.message))
            this.getColor()
          } else {
