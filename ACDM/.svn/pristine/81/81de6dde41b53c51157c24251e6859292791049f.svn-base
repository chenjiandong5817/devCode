  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_airportContact_list', click: getTableData}" size="small">查询</el-button>
        </el-form-item>
        <el-form-item v-auth="'post_airportContact_add'">
          <el-button type="primary" @click="handleAdd" size="small">新增</el-button>
        </el-form-item>
        <el-form-item v-auth="'put_airportContact_update'">
          <el-button type="primary" @click="handleEdit" :disabled="!rowSelected" size="small">编辑</el-button>
        </el-form-item>
        <el-form-item v-auth="'delete_airportContact_remove'">
...
export default {
  ...
  mounted () {
    ...
    this.tableOption = {
      ...
      column: [ /* eslint-disable */
        {
          prop: 'title', label: '职务', minWidth: 120, labelWidth: 90, span:24, query: 1,
          rules: [{ required: true, message: '请输入职务', trigger: 'blur' }]
        },
        {
          prop: 'content', label: '联系方式', minWidth: 120, labelWidth: 90, span:24,
          rules: [{ required: true, message: '请输入联系方式', trigger: 'blur' }]
        },
        {
          prop: 'airportId', label: '机场', minWidth: 120, labelWidth: 90, span:24, type: 'select',
          dicData: RemoteData.instance('get_baseAirport_list', { pageSize: 10000, pageNumber: 0 },
          (res) => res.data.content.map(item => { return { label: item.airportCNName, value: item.airportCNName } })),
          rules: [{ required: true, message: '请选择机场', trigger: 'change' }]
        }
      ]
    }
  },
  methods: {
    getTableData () {
      ...
      let getDataAjax = this.$auth('get_airportContact_list')
    ...
    handleRowSave (row, done) {
      let add = this.$auth('post_airportContact_add')
      add(row).then((res) => {
    ...
    handleRowUpdate (row, done) {
      let update = this.$auth('put_airportContact_update')
      update(row).then((res) => {
        res.status && this.$ok(res.message)
        this.getTableData()
        done()
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_airportContact_remove')
