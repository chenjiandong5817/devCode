  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_base_ckCounterOpMode_list'}" @click="getTableData('click')" size="small">查询</el-button>
        </el-form-item>
      ...
      <template slot="opmodeCodeForm" slot-scope="scope">
        <el-input v-model="scope.form[scope.column.prop]" @input="val => toUpperCase(val, scope.column)" :maxlength='1' placeholder="请输入编码" size="small"></el-input>
      </template>
    </crud>
  </div>
</template>
...
export default {
        ...
        column: [
          {
            ...
            prop: 'opmodeCode', label: '编码', midWidth: 50, query: 1, sortable: true,
            rules: [{ required: true, min: 1, max: 1, message: '请输入一位大写字母', trigger: 'change' }],
            meta: { convert: 'toUpperCase' }
          },
          { prop: 'description', label: '描述', span: 24, type: 'textarea', rules: [{ required: true, message: '请输入少于100位的描述', trigger: 'blur' }, { max: 100, message: '请输入少于100位的描述' }] },
          { prop: 'remark', label: '备注', span: 24, type: 'textarea', rules: [{ trigger: 'change' }, { max: 100, message: '请输入少于100位的备注' }] }
        ]
      },
      tableData: [],
      filters: {},
  ...
  methods: {
    getTableData (val) {
      ...
      let getDataAjax = this.$auth('get_base_ckCounterOpMode_list')
    ...
    handleRowSave (row, done) {
      ...
      let add = this.$auth('post_base_ckCounterOpMode_add')
    ...
    handleRowUpdate (row, done) {
      let formCopy = Object.assign({}, row)
      let update = this.$auth('put_base_ckCounterOpMode_update')
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_base_ckCounterOpMode_remove')
