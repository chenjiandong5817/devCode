  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_base_flightTask_list'}" @click="getTableData('click')" size="small">查询</el-button>
        </el-form-item>
      ...
      <template slot="flightTaskCodeForm" slot-scope="scope">
        <el-input v-model="scope.form[scope.column.prop]" @input="val => toUpperCase(val, scope.column)" :maxlength='3' placeholder="请输入航班任务代码" size="small"></el-input>
      </template>
    </crud>
  </div>
</template>
...
export default {
        ...
        column: [
          {
            ...
            prop: 'flightTaskCode', label: '航班任务代码', minWidth: 40, span: 24, sortable: true, query: 1,
            rules: [{ required: true, min: 1, max: 3, message: '请输入不超过3位的航班任务代码', trigger: 'blur' }],
            meta: { convert: 'toUpperCase' }
          },
          { prop: 'abbr1w', label: '一字简称', width: 100, rules: [{ trigger: 'blur' }, { max: 10, message: '请输入少于10位的一字简称' }] },
          { prop: 'abbr2w', label: '二字简称', width: 100, rules: [{ trigger: 'blur' }, { max: 20, message: '请输入少于20位的二字简称' }] },
          { prop: 'priority', label: '优先级', width: 80, type: 'number', meta: { max: 99, min: 0 } },
          { prop: 'disabled', label: '状态', width: 80, type: 'select', dicData: 'OPEN_OR_CLOSE', meta: { value: '0' } },
          { prop: 'description', label: '描述', span: 24, type: 'textarea', rules: [{ required: true, message: '请输入少于50位字符串的描述', trigger: 'blur' }, { max: 100, message: '请输入少于50位字符串的描述' }] },
  ...
  methods: {
    getTableData (val) {
      ...
      let getDataAjax = this.$auth('get_base_flightTask_list')
    ...
    handleRowSave (row, done) {
      ...
      let add = this.$auth('post_base_flightTask_add')
    ...
    handleRowUpdate (row, done) {
      let formCopy = Object.assign({}, row)
      let update = this.$auth('put_base_flightTask_update')
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_base_flightTask_remove')
