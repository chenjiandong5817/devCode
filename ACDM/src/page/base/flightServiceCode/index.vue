  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_base_flightServiceCode_list'}" @click="getTableData('click')" size="small">查询</el-button>
        </el-form-item>
      ...
      <template slot="flightServiceCodeForm" slot-scope="scope">
        <el-input v-model="scope.form[scope.column.prop]" @input="val => toUpperCase(val, scope.column)" :maxlength='50' placeholder="请输入服务代码" size="small"></el-input>
      </template>
    </crud>
  </div>
</template>
...
export default {
        ...
        column: [
          {
            ...
            prop: 'flightServiceCode', label: '服务代码', minWidth: 80, span: 24, sortable: true, query: 1,
            rules: [{ required: true, min: 1, max: 50, message: '请输入不超过50位的服务代码', trigger: 'blur' }],
            meta: { convert: 'toUpperCase' }
          },
          { prop: 'flightServiceDesc', label: '中文描述', span: 24, width: 120, query: 1, rules: [{ required: true, validator: validChinese, message: '请输入少于50位的中文描述', trigger: 'blur' }, { max: 50, message: '请输入少于50位的中文描述' }] },
          { prop: 'parentServiceId', label: '父级', span: 12, width: 80, type: 'number', meta: { max: 999999999, min: 0 } },
          { prop: 'serviceLevel', label: '层级', span: 12, width: 80, type: 'number', meta: { max: 99, min: 0 } },
          { prop: 'isDefault', label: '是否默认服务', span: 24, width: 120, type: 'select', dicData: 'YES_OR_NO', meta: { value: '0' } },
  ...
  methods: {
    getTableData (val) {
      ...
      let getDataAjax = this.$auth('get_base_flightServiceCode_list')
    ...
    handleRowSave (row, done) {
      ...
      let add = this.$auth('post_base_flightServiceCode_add')
    ...
    handleRowUpdate (row, done) {
      let formCopy = Object.assign({}, row)
      let update = this.$auth('put_base_flightServiceCode')
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_base_flightServiceCode')
