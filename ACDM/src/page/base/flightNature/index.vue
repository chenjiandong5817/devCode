  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_flightNature_list', click: getTableData}" size="small">查询</el-button>
        </el-form-item>
        <el-form-item v-auth="'post_flightNature_add'">
          <el-button type="primary" @click="handleAdd" size="small">新增</el-button>
        </el-form-item>
        <el-form-item v-auth="'put_flightNature_update'">
          <el-button type="primary" @click="handleEdit" :disabled="!rowSelected" size="small">编辑</el-button>
        </el-form-item>
        <el-form-item v-auth="'delete_flightNature_remove'">
...
export default {
    ...
    this.tableOption = {
      ...
      column: [
        { // eslint-disable-next-line
          prop: 'flightNatureCode', label: '编码', width: 150, labelWidth: 60, query: 1, span: 24,
  ...
  methods: {
    getTableData () {
      ...
      let getDataAjax = this.$auth('get_flightNature_list')
    ...
    handleRowSave (row, done) {
      let add = this.$auth('post_flightNature_add')
      add(row).then((res) => {
    ...
    handleRowUpdate (row, done) {
      let update = this.$auth('put_flightNature_update')
      update(row).then((res) => {
        res.status && this.$ok(res.message)
        this.getTableData()
        done()
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_flightNature_remove')
