  ...
  <div>
    <crud
      ...
      <template slot="btn_opt">
        <el-form-item>
          <el-button type="primary" v-auth="{code: 'get_flightDirection_list', click: getTableData}" size="small">查询</el-button>
        </el-form-item>
        <el-form-item v-auth="'post_flightDirection_add'">
          <el-button type="primary" @click="handleAdd" size="small">新增</el-button>
        </el-form-item>
        <el-form-item v-auth="'put_flightDirection_update'">
          <el-button type="primary" @click="handleEdit" :disabled="!rowSelected" size="small">编辑</el-button>
        </el-form-item>
        <el-form-item v-auth="'delete_flightDirection_remove'">
    ...
    this.tableOption = {
      ...
      column: [
        { // eslint-disable-next-line
          prop: 'directionCode', label: '编码', width: 150, labelWidth: 60, query: 1, span: 24,
          rules: [{ required: true, message: '请输入编码', trigger: 'blur' }, { max: 1, message: '超出一位限制' }]},
        { // eslint-disable-next-line
          prop: 'description', label: '描述', minWidth: 120, labelWidth: 60, span: 24,
          rules: [{ required: true, message: '请输入描述', trigger: 'blur' }, { max: 2, message: '超出二位限制' }]
        },
        { prop: 'remark', label: '备注', minWidth: 120, labelWidth: 60, span: 24, type: 'textarea' }
      ]
    }
  },
  methods: {
    getTableData () {
      ...
      let getDataAjax = this.$auth('get_flightDirection_list')
    ...
    handleRowSave (row, done) {
      let add = this.$auth('post_flightDirection_add')
      add(row).then((res) => {
    ...
    handleRowUpdate (row, done) {
      let update = this.$auth('put_flightDirection_update')
      update(row).then((res) => {
        res.status && this.$ok(res.message)
        this.getTableData()
        done()
    ...
    handleRowDelete (row) {
      this.$confirm(`是否删除?`, {
        ...
        let remove = this.$auth('delete_flightDirection_remove')
