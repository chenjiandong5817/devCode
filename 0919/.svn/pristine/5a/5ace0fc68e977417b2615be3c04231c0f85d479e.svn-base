  ...
  <el-form :ref="setFormId(index)" :model="form.resLists[index]" :label-position="labelPosition" :rules="rules">
    <el-row :gutter="18" v-for="(rowItem, rowIndex) in resFields" :key="rowIndex" class="content">
      <el-col :span="6" v-for="(ritem, cIndex) in rowItem" :key="ritem.name" v-if="!ritem.hidden">
        <el-form-item :label="ritem.label" :prop="ritem.name" :rules="ritem.rules" :key="item.id" v-if="!ritem.hidden">
          ...
          <el-radio-group v-model="form.resLists[index][ritem.name]" v-else-if="ritem.type === 'radio'">
            <el-radio v-for="(radioItem, radioIndex) in ritem.choose" class="radio" :label="radioItem.value" :key="radioIndex">{{radioItem.text}}</el-radio>
          </el-radio-group>
          <!-- select判断 -->
          <el-select class="selectA" v-model="form.resLists[index][ritem.name]" :filterable="ritem.filterable" :clearable="ritem.clearable" :disabled="isDisabled(index)" placeholder="请选择" v-else-if="ritem.type === 'select' && !ritem.isLink && !ritem.isSetChange" >
            <el-option
              v-for="(selectItem, selIndex) in ritem.choose"
              :key="selIndex"
          ...
          <el-select class="selectA" v-model="form.resLists[index][ritem.name]" :filterable="ritem.filterable" :clearable="ritem.clearable" :disabled="isDisabled(index)" placeholder="请选择" v-else-if="ritem.type === 'select' && !ritem.isLink && ritem.isSetChange" @change="setLinkVal(ritem.changeFun, form.resLists[index][ritem.name], index, rowIndex, cIndex)">
            <el-option
              v-for="(selectItem, selIndex) in resFieldList[index][rowIndex][cIndex].choose"
              :key="selIndex"
...
export default {
    ...
    counterConf: function () {
      ...
      if (this.curTabName === 'counterConfForm') {
        array = [
          ...
          { name: 'terminal', value: this.flight.terminal, label: '航站楼', selVal: 'terminalCode', selText: 'terminalCode', type: 'select', filterable: true, clearable: true, choose: this.baseList.terminalList, rules: [ { required: true, message: '请选择航站楼' } ], isLink: false, isSetChange: true, changeFun: 'setLinkLs', linklsName: 'counterGroupls', linkRIndex: 0, linkCIndex: 8, linkField: 'terminal' },
          ...
          { name: 'scheduleOpen', value: this.date, label: '开办时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '开办日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] },
          { name: 'scheduleClose', value: this.date, label: '停办时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '停办日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] }]
        ]
      } else {
    ...
    gateConf: function () {
      ...
      if (this.curTabName === 'gateConfForm') {
        array = [
          ...
          { name: 'scheduleOpen', value: this.date, label: '开始时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '开始日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] },
          { name: 'scheduleClose', value: this.date, label: '结束时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '结束日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] }]
        ]
      } else {
    ...
    carConf: function () {
      ...
      if (this.curTabName === 'carConfForm') {
        array = [
          ...
          { name: 'terminal', value: this.flight.terminal, label: '航站楼', selVal: 'terminalCode', selText: 'terminalCode', type: 'select', filterable: true, clearable: true, choose: this.baseList.terminalList, rules: null, isLink: true, isSetChange: true, changeFun: 'setLinkLs', linklsName: 'carList', linkRIndex: 0, linkCIndex: 6, linkField: 'terminal' },
          ...
          { name: 'scheduleOpen', value: this.date, label: '开启时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '开启日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] },
          { name: 'scheduleClose', value: this.date, label: '关闭时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '关闭日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] }]
        ]
      } else {
        return this.carConf
      }
    ...
    standConf: function () {
      ...
      if (this.curTabName === 'standConfForm') {
        array = [
          ...
          { name: 'estOccupiedBegin', value: this.date, label: '开始时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '开始日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] },
          { name: 'estOccupiedEnd', value: this.date, label: '结束时间', visitTime: true, type: 'dateTimeGroup', datePlaceholder: '结束日期', timePlaceholder: '时间', formatter: 'yyyy-MM-dd', rules: [ { validator: Rules.validSegTimeType, message: '非法时间格式', trigger: 'blur' } ] }]
        ]
      } else {
  ...
  methods: {
      ...
      if (JSON.stringify(newlist) !== JSON.stringify(oldlist)) {
        ...
        if (this.validForm()) {
          ...
          if (this.curTabName === 'counterConfForm') {
            ...
            // 3.值机柜台资源分配时间检验
            // console.log(this.ckinCounterVerify(param))
            this.ckinCounterVerify(param)
            // console.log(result)
            // if (result === -1) {
            ...
            // if (result.statue !== 1) {
            //   // 提示值机柜台资源时分配错误
            //   this.$notify(Util.notifyBody(false, result.msg))
            //   // return false
            // } else {
            //   this.to(param).then((res) => {
            //     this.loading = false
    ...
    ckinCounterVerify (param) {
      ...
      let result = { status: 1, msg: '值机柜台配置正确' }
        ...
        if (data.ok) {
          ...
          for (var i = 0; i < resultList.length; i++) {
            ...
            if (resultList[i].status === -1) {// 错误提示方式待定 1107
              result = { status: resultList[i].status, msg: resultList[i].message }
              this.$notify(Util.notifyBody(false, result.msg))
              return false
