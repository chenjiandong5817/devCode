  ...
  export default {
      ...
      return {
        ...
        fields: [
          ...
          },
          {
            id: '2',
            item: [
               { name: 'carouselCode', value: '', label: '编码', type: 'text', rules: [ { max: 6, min: 1, required: true, message: '请输入一位到六位字符串的编码' } ], placeholder: '编码', span: 12 },
               { name: 'carouselType', value: '', label: '转盘类型', type: 'select', choose: [ { text: '进', value: 'A' }, { text: '出', value: 'D' } ], rules: [ { required: true, message: '请选择行李转盘类型' } ], placeholder: '', span: 12 }
            ]
          },
          {
            id: '3',
            item: [
               { name: 'carouselSize', value: '', label: '大小', type: 'text', rules: [ { max: 1, min: 1, message: '请输入一位字符串的大小' } ], placeholder: '', span: 12 },
    ...
    methods: {
      ...
      handleAdd: function () {
        ...
        this.fields[3].item[0].rules = this.carouselSizeValidate
        this.fields[3].item[1].choose = this.flightNatureChoose
        this.fields[4].item[0].choose = this.flightStatusChoose
      ...
      getCarouselSizeValidate: function () {
        this.carouselSizeValidate = (rule, value, callback) => {
          if (value < 0) {
            return callback(new Error('大小不能为负数'))
          }
          if (value > 10) {
            return callback(new Error('输入的值不能为两位数'))
          }
          callback()
        }
      },
