  ...
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="filters">
        <el-form-item>
        <el-select v-model="filters.system" placeholder="请选择操作系统类型">
        <el-option
          v-for="item in systems"
          :key="item.id"
          :label="item.name"
    ...
    <!--分页-->
    <pagination :to="getVersions" ref="page"></pagination>
    <!--删除窗口-->
    <common-delete
  ...
  export default {
    ...
    methods: {
      ...
      sendDirective: function () {
        let msg = JsonList.upgrade
        // 配置好后发送
        let airportCodeArray = this.uploadParams.airport.split(',')
        for (var i = 0; i < airportCodeArray.length; i++) {
          let routingKey = 'ROOT' + '.' + airportCodeArray[i]
          StompUtil.handleSend(routingKey, msg)
        }
      }
    },
    mounted () {
