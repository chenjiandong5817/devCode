 ...
 * @Last Modified time: 2018-01-02 10:43:05
 * 播放列表管理模块树形结构
 */
...
import DisplayAddOrUpdate from './../../fids/DisplayManager/DisplayAddOrUpdate'
import DisplayMuiltAdd from './../../fids/DisplayManager/DisplayMuiltAdd'
...
export default {
  ...
  methods: {
    ...
    callbackSend () {
      this.$refs['display'].getParentSendIp(this.clickIp, this.airportCode)
    },
    terminalNodeClick (data) {  // 以航站楼为根节点的树形控件点击事件
      if (data.children.length === 0) {
        let terminalIp = this.ipMap.get(data.label)
        this.clickIp = terminalIp
        this.$refs['display'].getParentSendIp(terminalIp, this.airportCode)
      }
    ...
    devTypeNodeClick (data) {   // 在里面做每个Item的点击事件
      if (data.children.length === 0) { // 叶子节点点击才有用
        ...
        this.$refs['display'].getParentSendIp(devTypeIp, this.airportCode)
      }
