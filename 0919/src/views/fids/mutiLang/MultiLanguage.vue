 ...
 * @Last Modified time: 2017-12-28 10:06:08
 */
<template>
        ...
        <div>
          <el-form-item :span="4">
            <el-select v-model="filters.originaltype" placeholder="语言类型" @change="cli">
              <el-option
                v-for="item in originaltypeChoose"
                :key="item.value"
                :label="item.label"
                :value="item.value">
  ...
  export default {
    data () {
      return {
        ...
        sels: [],
        originaltypeChoose: [
           { label: 'AIRPORT', value: 'AIRPORT', text: '机场' },
           { label: 'AIRLINE', value: 'AIRLINE', text: '航线' },
           { label: 'FLIGHTSTATUS', value: 'FLIGHTSTATUS', text: '航班状态' },
           { label: 'LABEL', value: 'LABEL', text: '标签' },
           { label: 'LABELTEXT', value: 'LABELTEXT', text: '标签文本' },
           { label: 'AREA', value: 'AREA', text: '区域' }
        ],
        langcodeChoose: [
           { label: 'zh-cn', value: 'zh-cn', text: '中文' },
           { label: 'en-ww', value: 'en-ww', text: '英文' },
           { label: 'ja-jp', value: 'ja-jp', text: '日文' }
        ],
        // 新增编辑需要的字段
        fields: [
