 ...
 * @Last Modified time: 2018-01-12 15:34:01
 */
<template>
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="6">
          <el-form-item label="航站楼" prop="terminal" :rules="[ { required: true, message: '请选择航站楼', trigger: 'blur' } ]">
            <el-select v-model="form.segment.terminal" placeholder="航站楼" style="width:100%;" filterable clearable :disabled="editAble">
              <el-option
        ...
        <el-col :span="6">
          <el-form-item label="状态" prop="flightStatus">
            <el-select v-model="form.segment.flightStatus" placeholder="状态" style="width:100%;" filterable clearable  :disabled="editAble || operate === 'addAlt'">
              <el-option
                v-for="(item, index) in baseList.flightstatus"
                :key="index"
        ...
        <el-col :span="18" v-if="isShowIrrInfo">
          <el-form-item label="异常原因" prop="irrReason">
            <el-select v-model="form.irrReason" placeholder="异常原因" style="width: 100%;" filterable>
              <el-option
                v-for="(item, index) in baseList.irregularcodes"
                :key="index"
                :label="item.description"
...
export default {
  ...
  data () {
    return {
      ...
      firstSeg: null,
      lastSeg: null,
      baseList: {terminals: [], generalagents: []},
      registrations: [],
      allRegistrationList: [],
  ...
  methods: {
    ...
    setIrrInfo () {
      if (this.operate === 'flightCnl') {
        this.form.segment.irregularStatus = 'CNL'
      } else if (this.operate === 'flightDly') {
        this.form.segment.irregularStatus = 'DLY'
        // this.baseList['irregularcodes'] = this.$cache.fetch('irregularcodes')
      } else {
        // this.baseList['irregularcodes'] = this.$cache.fetch('irregularcodes')
      }
      // 过滤异常原因分组100000
      this.baseList['irregularcodes'] = this.setIrrList('100000')
    },
    setIrrList (superior) {
      let list = this.$cache.fetch('irregularcodes').filter(item => {
        return item.superior === superior && item.superior !== null
      })
      return list
    },
    ...
    bindCurStation (segment, index, segLength) {
      ...
      let obj = {}
      let segList = Util.deepCopy(this.$refs['airlineForm'].getAirlineList().newAirline)
      if ((segment.direction === 'A' && index === segLength - 1) || (segment.direction === 'D' && segList[0].origin === segment.origin)) {
    ...
    setReplaceList: function () {
      this.loading = true
      // 0910 从缓存中获取基础表信息
      let tkeys = ['airports', 'flightdirections', 'generalagents', 'airlines', 'flighttasks', 'flightstatus', 'aircraftTypes', 'stand', 'gate', 'vipranks']
    ...
    setAirportCode (airportCode) {
      this.form.segment.airportCode = airportCode
      this.setLinkFields(this.form.segment.airportCode)
    },
