 ...
 * @Last Modified time: 2018-01-12 16:27:15
 */
<template>
<div class="resClass">
  <div :class="widthCls">
        ...
        <el-col :span="6">
          <el-form-item label="始发站" prop="origin">
            <el-select v-model="form.segment.origin" style="width:100%;" filterable placeholder="始发站" clearable :disabled="true">
              <el-option
                v-for="(item, index) in this.$cache.fetch('airports')"
                :key="index"
...
export default {
  ...
  methods: {
    ...
    handleClick: function (tab, event) {
      ...
      // 航站楼、登机口、机位、转盘关联运营机场
      let terminalList = this.$cache.findListByName('terminals', 'airportCode', this.form.segment.airportCode)
      let baseList = { counterGroupls: this.counterGroupls, ckinGroup: ckinGroup, counterList: this.counterList, gateList: this.gateList, carList: this.carList, standList: this.standList, terminalList: terminalList, ckcouOpmodeLs: this.$cache.fetch('ckcounteropmodes'), ckCounterSerTypes: this.$cache.fetch('ckcounterservicetypes') }
