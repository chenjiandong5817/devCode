 ...
 * @Last Modified time: 2018-01-12 13:33:08
 */
<template>
<div class="seaScheduleFormcls">
  ...
  <el-dialog :title="title" v-model="visible" :close-on-click-modal="false" @close="handleClose" :modal="false" @open="setClassName">
    <el-form :model="form.seasonSeg" :rules="rules" label-position="right" label-width="80px" ref="seaScheduleForm">
      ...
      <el-row :gutter="2">
        ...
        <el-col :span="6">
          <el-form-item label="进出港" prop="direction">
            <el-select v-model="form.seasonSeg.direction" name="direction" placeholder="进出" clearable filterable @change="bindAirport">
              <el-option
                v-for="(item, index) in baseList.flightdirections"
                :key="index"
                :label="item.description"
      ...
      <el-row :gutter="2">
        <el-col :span="24">
          <el-form-item label="航季周期" label-width="80px" class="labelCls">
      ...
      <el-row :gutter="2">
        <el-col :span="6">
          <el-form-item
            ...
            <el-select v-model="form.airSegList[index].origin" filterable placeholder="始发站" clearable @change="setFlightNatureVal(index, form.airSegList[index].destination)">
              <el-option
                v-for="(item, index) in baseList.airports"
                :key="index"
        ...
        <el-col :span="2">
          <el-form-item label="区域属性"  prop="flightNature">
            <el-select v-model="form.airSegList[index].flightNature" placeholder="区域" style="width: 109%; color: #fff;" filterable remote clearable :disabled="true">
              <el-option
                v-for="(item, index) in $cache.fetch('flightnatures')"
                :key="index"
                :label="item.description"
...
import shareFlight from '../../planFlightSchedule/planFlightForm/ShareFlightForm'
...
export default {
  ...
  data () {
    return {
      ...
      airSegment: {id: null, flightId: null, origin: '', destination: '', flightNature: '', segmentNature: '', scheduleDepartTime: '', scheduleArriveTime: '', depOffset: 0, arrOffset: 0},
      isCheckAll: false,
  ...
  methods: {
    ...
    setSeasonList (airportCode) {
      ...
      this.form.seasonSeg.generalAgent = ''
    },
    ...
    initData (airportCode) {
      ...
      this.form.seasonSeg.flightTask = 'W/Z'
    ...
    setFlightNature (index) {
      // 设置航班属性
      let originList = this.baseList.airports.find((item) => {
          return item.icaocode === this.form.airSegList[index].origin
      })
      let originNature = (originList !== undefined ? originList.airportnature : null)
      let desList = this.baseList.airports.find((item) => {
          return item.icaocode === this.form.airSegList[index].destination
      })
    ...
    setFlightNatureVal (index, val) {
      ...
      // 设置下个航段的起飞地
      if (index + 1 < this.form.airSegList.length) {
         this.form.airSegList[index + 1].origin = val
      }
    },
    showShareFlight () {
      // 展示共享航班编辑界面
    ...
    addAirSegList (index) {
      ...
      // 若为进港航班、则联动设置航段区域属性
      if (this.form.seasonSeg.direction === 'A') {
        this.form.airSegList[index + 1].flightNature = this.setFlightNature(index + 1)
      }
    },
    delAirSegment (index) {
    ...
    bindAirport () {
      // 设置航线绑定起飞目的地
      let airportCode = this.form.seasonSeg.airportCode
      if (airportCode !== null && airportCode !== undefined && airportCode !== '' && airportCode.length === 3) {
        airportCode = this.$cache.findByName('airports', 'iatacode', airportCode, 'icaocode')
      }
      if (this.form.seasonSeg.direction === 'A') {
        // 进港 1215
        this.airSegment.origin = ''
        this.airSegment.destination = airportCode
        if (this.form.airSegList.length > 0) {
          this.form.airSegList[this.form.airSegList.length - 1].destination = airportCode
        }
      } else if (this.form.seasonSeg.direction === 'D') {
        // 出港
        this.airSegment.origin = airportCode
        this.airSegment.destination = ''
        if (this.form.airSegList.length > 0) {
          this.form.airSegList[0].origin = airportCode
        }
      }
    }
  },
