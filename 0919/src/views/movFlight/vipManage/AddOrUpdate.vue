  ...
  methods: {
  	...
  	show: function (data) {
      	...
      	if (this.formData.vipDetails.length === 1) {
      		if (this.formData.vipDetails[0].vipName.length === 0) {
    ...
    getDynamicFlight: function () {
	 ...
	 // 获取VIP类型下拉框数据
      ...
      delVIPDetail: function (itemData) {
      	...
      	this.numberOfVipDetails = this.formData.vipDetails.length
      	// 增加判断
      	if (this.formData.vipDetails[this.numberOfVipDetails - 1].vipName.length === 0) {
      ...
      handleSubmit: function () {
      	...
      	var validTag = null
      	for (var i = 0; i < this.formData.vipDetails.length; i++) {
      		if (this.formData.vipDetails[i].vipName.length !== 0) {
      			validTag = 'success'
      		} else {
      			validTag = 'error'
      			break
      	...
      	if (validTag === 'error') {
      		this.$notify(Util.notifyBody(false, 'vip详细信息中的姓名不能为空！'))
      	}
      },
      // 转换时间格式
