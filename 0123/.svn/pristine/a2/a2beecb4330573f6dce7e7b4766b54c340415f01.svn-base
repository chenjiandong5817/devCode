...
export default {
    ...
    if (conf instanceof Array) {
      ...
      for (var key in conf) {
        if (key === 'dataSource') {
          apis.push({
            key: conf[key],
            mainPager: conf['mainPager'],
            mainSource: conf['mainSource'],
            singleData: conf['singleData'],
            refreshInterval: conf['refreshInterval'] && conf['refreshInterval'] > -1 ? conf['refreshInterval'] : 20000,
            pageInterval: conf['pageInterval'] && conf['pageInterval'] > -1 ? conf['pageInterval'] : 20000
          })
        } else {
  ...
  completeStyleSuffix (styleObj) {
    ...
    const regex = /width|height|^padding|^margin|fontSize|borderRadius|^border[\w]*Width$/gi
  ...
  // 生成图片路径, 多字段拼接, 20180109 修改静态图片获取（静态图片不需要图片类型）
  genImgPath (conf, data, airportCode) {
    // if (!(conf.columnName && conf.imgType && conf.imgSize)) {
    //   return ''
    // }
    let path = `?airportcode=${airportCode}`
    if (conf.imgType) {
      path += `&imagetype=${conf.imgType}`
    }
    if (conf.imgSize) {
      path += `&imagesize=${conf.imgSize}`
    }
    ...
    if (conf.columnName.includes(',')) {
      ...
      conf.columnName.split(',').forEach(name => {
        nameArray.push(this.getObjectValue(conf.static ? name : data[name], conf.property) || 'undefined')
      })
      path += `&imagename=${nameArray.join('_')}`
    } else {
      path += `&imagename=${this.getObjectValue(conf.static ? conf.columnName : (typeof data === 'object' ? data[conf.columnName] : data), conf.property)}`
    }
    return path
  },
  // multiSource
  ...
  // 给具有columnName的配置设置flopItemName
  coverFlopValue (conf, name, value) {
    if (conf instanceof Array) {
      for (let item of conf) {
        this.coverFlopValue(item, name, value)
      }
    } else {
      for (let key in conf) {
        if (key === 'columnName') {
